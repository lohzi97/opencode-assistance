...
3. Review changelogs for every outdated component with integration risk. Sources:
   - `opencode`: GitHub releases for `anomalyco/opencode` (range: current to latest); installed via the official installer, updated by `./update.sh` re-running it.
...
8. If `opencode`, any MCP repo, or `@openchamber/web` changed, remind Master that the running stack still uses old binaries and offer the restart-opencode skill flow. Do not restart on your own initiative beyond that.
