---
description: Read through proposal documents and prepare for an informed discussion about a change
---
...
**Input**: Optionally specify a change name after `/opsx-discuss` (e.g., `/opsx-discuss add-auth`). If omitted, check if it can be inferred from conversation context. If vague or ambiguous you MUST prompt for available changes.
...
1. **Select the change**
   ...
   Always announce: "Preparing discussion for: <name>" and how to override (e.g., `/opsx-discuss <other>`).
...
This command supports the "discuss before deciding" workflow:
...
- **Bridges to other commands**: After discussion, the user may want to run `/opsx-review-proposal`, `/opsx-apply-resume`, `/opsx-align`, etc.
