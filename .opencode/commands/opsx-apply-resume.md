---
description: Review implementation status, run validations, fix issues, and continue applying remaining tasks
---
...
**Input**: Optionally specify a change name (e.g., `/opsx-apply-resume add-auth`). If omitted, check if it can be inferred from conversation context. If vague or ambiguous you MUST prompt for available changes.
...
1. **Select the change**
   ...
   Always announce: "Resuming change: <name>" and how to override (e.g., `/opsx-apply-resume <other>`).
