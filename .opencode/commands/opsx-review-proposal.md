---
description: Review a change proposal against the codebase, clarify requirements, and update proposal files
---
...
**Input**: Optionally specify a change name after `/opsx-review-proposal` (e.g., `/opsx-review-proposal add-auth`). If omitted, check if it can be inferred from conversation context. If vague or ambiguous you MUST prompt for available changes.
...
1. **Select the change**
   ...
   Always announce: "Reviewing proposal: <name>" and how to override (e.g., `/opsx-review-proposal <other>`).
...
9. **Confirm proposal readiness**
   ...
   → Run `/opsx-review-proposal <name>` again when ready
   ```
