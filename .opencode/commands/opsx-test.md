---
description: Perform end-to-end manual-style testing for a completed OpenSpec change
---
...
This command replaces the human manual testing phase in the OpenSpec workflow. It must determine whether it has enough tool capability to perform meaningful end-to-end testing, stop immediately if a required capability is missing, then execute happy-path and edge-case tests as thoroughly as possible.

**Input**: Optionally specify a change name after `/opsx-test` (e.g., `/opsx-test add-auth`). If omitted, check if it can be inferred from conversation context. If vague or ambiguous you MUST prompt for available changes.
...
1. **Select the change**
   ...
   Always announce: "Testing change: <name>" and how to override (e.g., `/opsx-test <other>`).
...
2. **Check status to understand the schema**
   ...
   If tasks are incomplete, continue only if the user explicitly asked to test a partial implementation. Otherwise report that the change is not ready for E2E testing and suggest `/opsx-apply-resume <name>`.
...
7. **Run prerequisite validation**
   ...
   Do not stop at automated tests. This command's purpose is end-to-end validation. Use automated tests only as supporting evidence.
...
13. **Provide final E2E report**
   ...
   ### Recommendation
   - [Archive-ready, needs `/opsx-fix`, needs `/opsx-align`, needs missing test capability, etc.]
   ```
...
- Clean up only test artifacts created during this command.
- If the implementation appears correct but the proposal is outdated, recommend `/opsx-align` rather than changing artifacts.
- If a bug is found, recommend `/opsx-fix` with the concrete failure description.
- Use contextFiles from CLI output, don't assume specific file names.
- Refer to `openspec/AGENTS.md` (located inside the `openspec/` directory — run `ls openspec` or `openspec update` if you don't see it) if you need additional OpenSpec conventions or clarifications.

**Fluid Workflow Integration**

This command supports the manual-testing replacement stage:

- Use after `/opsx-apply-resume` reports that implementation matches the proposal.
- Use before `/opsx-align` to discover UIUX or behavioral drift that should be reflected in proposal artifacts.
- Use before `/opsx-archive` to decide whether the change is archive-ready.
- If testing finds bugs, use `/opsx-fix <change-name> <issue>`.
- If testing finds intentional proposal drift, use `/opsx-align <change-name>`.
