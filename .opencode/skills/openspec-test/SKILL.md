...
license: MIT
compatibility: Requires OpenSpec change files and manual tester subagents.
metadata:
  author: ddone
  version: "1.0"
---
...
- "use openspec-test skill to perform testing on <change> proposal"
- "test this OpenSpec change and write findings to fix.md"
- "verify UI matches design for OpenSpec change <name>"

---

**Input**: A change name (kebab-case) such as `dialpad-ui-refresh`.
...
- This skill is for testing + issue documentation, not implementation.
...
Typical invocation from user:
- "Use openspec-test skill to perform testing on `dialpad-ui-refresh` proposal"

Expected behavior:
1. Read proposal/spec/design/tasks + referenced assets.
2. Run the correct tester subagent(s) with explicit requirements.
3. Write or update `openspec/changes/dialpad-ui-refresh/fix.md` with structured issues.
